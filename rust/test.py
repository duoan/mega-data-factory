...
try:
    import rust_accelerated_ops as ops
except ImportError:
    print("Error: rust_accelerated_ops not installed.")
    print("Run: cd rust && maturin build --release -o ../dist && uv pip install ../dist/*.whl")
    exit(1)
...
artifacts, entropy = ops.assess_quality(img_bytes.getvalue())
...
results = ops.assess_quality_batch(images)
print(f"✓ Test 3 (batch): processed {len(results)} images")

# Test 4: Perceptual hash
phash = ops.compute_phash(img_bytes.getvalue(), 16)
print(f"✓ Test 4 (phash): {phash[:20]}...")

# Test 5: Batch phash
phashes = ops.compute_phash_batch(images, 16)
print(f"✓ Test 5 (batch phash): computed {len(phashes)} hashes")
